...
module TagCommands
  ...
  TAG_MENU_OPTIONS = [
    ...
    { name: "Add a new tag to this tag", value: :add_tag },
  ...
  def view_tags
    results = load_tags
    display_tags_details(results) unless results.empty?
    manage_selected_card
  end

  def manage_tag
    manage_selected_tag if load_and_display_tag_choices
    manage_selected_card
  end
  ...
  def display_tags_details(tags)
    puts "\n=== Tags in #{@card['name']}==="
    tags.each_with_index { |tag, index| display_single_tag(tag, index) }
  end
  ...
  def tag_tag_display_message(tag)
    if tag["tags"].length.positive?
      tag["tags"].map do |tag|
        tag["name"]
      end.join(", ")
    else
  ...
  def show_no_tags_message
    @prompt.warn("📭 No tags found. Create your first tag!")
    []
  end
